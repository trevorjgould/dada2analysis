...
hist(rowSums(newtable), xlab = "Sequence Count of Sample", main = "Histogram Sequence Count")
...
if (sampler < 1000){
  ...
  p1 <- ggplot(scount, aes(x = scount)) + geom_dotplot(binwidth = 1000, aes(fill = col)) + geom_vline(xintercept = 1000, color = "red") + ylab("Samples") + xlab("Sequences") + scale_fill_manual(values = c("red","black"))
  print(p1)
} else {
...
## Create the tables used for all analysis
```{r}
...
tout1 <- lapply(comp,Taxonomy_Plots)
tout2 <- lapply(comp,Taxonomy_Plots_Top10)
```
...
comp <- colnames(newmap)
lout <- lapply(comp, Diversity_Plots)
```
