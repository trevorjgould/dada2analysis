...
Taxonomy_Plots <- function(meta){
  ...
  for (x in 1:7){
    ...
    #p2 <- ggplot(melted, aes(as.factor(Day),variable, fill=value)) + geom_tile(color="white", size=0.1) + xlab("Day") + ylab(name_label) + theme(axis.text.y = element_text(size = 8)) + scale_fill_gradientn(colours = pal,name = "Proportion")
...
taxa_list <- list(KT,PT,CT,OT,FT,GT,ST)
  ...
  for (x in 1:7){
    ...
    melted$variable <- gsub("[a-z]__", "", melted$variable)
    melted$variable <- gsub("\\.", " ", melted$variable)
    filename <- paste0(name_label,"_taxonomy_top10.png")
    filename2 <- paste0(name_label,"_taxonomy_top10.txt")
    titlename <- paste0("Top_",t,"_",name_label)
    forcolors <- c(rep(c("#899DA4", "#C93312"), times = 6))
    p <- ggplot2::ggplot(melted, ggplot2::aes(Samples, (value*100), fill = variable)) + ggplot2::geom_bar(stat='identity')+ ggplot2::ylab("Percent") + ggplot2::theme_bw() + ggplot2::theme(legend.position="none") + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)) + ggplot2::scale_fill_manual(values = forcolors, name = name_label) + facet_wrap(~variable, ncol = 1, scales = "free_y", strip.position = "left")+theme(strip.text.y.left = element_text(angle = 0)) + ggtitle(titlename)
    ggplot2::ggsave(p, file = filename, dpi  = 800, width = 12, height = 8, units = "in")
    write.table(both, file = filename2, sep = "\t", quote = FALSE)
    #melted$variable = with(melted, reorder(variable,value,mean))
    #p2 <- ggplot(melted, aes(as.factor(Day),variable, fill=value)) + geom_tile(color="white", size=0.1) + xlab("Day") + ylab(name_label) + theme(axis.text.y = element_text(size = 8)) + scale_fill_gradientn(colours = pal,name = "Proportion")
    #filename3 <- paste0(name_label,"_taxonomy_heat.png")
