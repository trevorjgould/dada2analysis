...
Taxonomy_Plots_Top10 <- function(t){
  # read in tables
  pal = c('#91bfdb','#ffffbf','#fc8d59')
  #meta <- read.table("Metadata_common.txt", sep = "\t", check.names = FALSE)
  KT <- read.table(file = "Kingdom_taxonomy.txt", sep = "\t", check.names = FALSE)
  ...
  if (ncol(XTtax) > t){   
    ...
    p <- ggplot2::ggplot(melted, ggplot2::aes(Samples, (value*100), fill = variable)) + ggplot2::geom_bar(stat='identity')+ ggplot2::ylab("Percent") + ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom") + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)) + ggplot2::scale_fill_discrete(name = name_label) + facet_wrap(~variable, ncol = 1, scales = "free_y", strip.position = "left")+theme(strip.text.y.left = element_text(angle = 0))
    ggplot2::ggsave(p, file = filename, dpi  = 800, width = 10, height = 8, units = "in")
    write.table(both, file = filename2, sep = "\t", quote = FALSE)
    #melted$variable = with(melted, reorder(variable,value,mean))
    #p2 <- ggplot(melted, aes(as.factor(Day),variable, fill=value)) + geom_tile(color="white", size=0.1) + xlab("Day") + ylab(name_label) + theme(axis.text.y = element_text(size = 8)) + scale_fill_gradientn(colours = pal,name = "Proportion")
