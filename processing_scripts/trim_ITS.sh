...
mkdir ../02_filtered  
#ITS
for i in *_R1_001.fastq.gz; do echo "cutadapt --cores 4 --pair-filter=any --minimum-length 100 --discard-untrimmed -g TCGATGAAGAACGCAGCG -G TCCTCCGCTTATTGATATGC -o ../02_filtered/${i} -p ../02_filtered/${i//_R1_001.fastq.gz/_R2_001.fastq.gz} ${i} ${i//_R1_/_R2_} > ../02_filtered/cutadapt.${i//_R1_001.fastq.gz/.adapter.log.txt}" >> run_cutadapt2.cmd; done
